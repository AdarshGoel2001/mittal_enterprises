...
export async function POST(req: NextRequest) {
  ...
  if (!name || !email || !phone || !category || !message) {
    return NextResponse.json(
      { error: 'Name, email, phone, product category and requirements are required.' },
      { status: 400 },
    );
  }
  ...
  if (!apiKey || !to) {
    console.error('Enquiry route misconfigured: missing RESEND_API_KEY or CONTACT_EMAIL.');
  ...
  const html = `<div style="font-family:-apple-system,Segoe UI,Helvetica,Arial,sans-serif;font-size:14px;line-height:1.6;color:#111;">
    <h2 style="margin:0 0 16px;font-size:16px;">New product enquiry</h2>
    ${fields
      .map(
        ([k, v]) =>
  ...
  try {
    ...
    const result = await resend.emails.send({
      ...
      to,
      replyTo: email,
      subject: `[Mittal Enterprises] New enquiry: ${productName || 'general'}`,
      html,
      text,
    });
    if (result.error) {
      console.error('Resend error (enquiry):', result.error);
      return NextResponse.json({ error: 'Failed to send enquiry.' }, { status: 502 });
    }
    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error('Enquiry send exception:', err);
